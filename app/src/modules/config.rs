...
use fastrixi_kernel::tools::{randstr, CharClass};
...
pub fn initialize_configs_dir() -> CommandResult<()> {
  ...
  let fastrixi_dir = home_dir.join(".fastrixi");
  if !fastrixi_dir.exists() {
    if fs::create_dir_all(&fastrixi_dir).is_err() {
      failed!("fastrixi directory is missing");
    }
  }

  let configs_dir = fastrixi_dir.join("configs");
