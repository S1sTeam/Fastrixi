...
class InfoModule {
	...
	public async init(): Promise<void> {
		try {
			...
			addOpeningUrlTo("codeberg", "click", "https://github.com/S1sTeam/Fastrixi");
	...
	private async lnf(): Promise<LnfResult> {
		try {
			const repository = await invoke<any>("download_json", {
				url: "https://api.github.com/repos/S1sTeam/Fastrixi"
			});

			if (!repository) return { data: null, error: "resource moved or unavailable" };

			const social = await invoke<any>("download_json", {
				url: "https://raw.githubusercontent.com/S1sTeam/Fastrixi/main/fastrixi.social.json"
			});
