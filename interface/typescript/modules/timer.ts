...
class TimerModule {
	...
	public init(): void {
		...
		setInterval(() => {
			...
			switch (time) {
				case "30:00":
					messages.message("fastrixi", `Ого, ты активен уже 30 минут! Не хочешь передохнуть?`);
					break;
				case "01:00:00":
					messages.message("fastrixi", `Надо же, ты активен уже 1 час! Не хочешь передохнуть?`);
					break;
				case "02:00:00":
					messages.message("fastrixi", `Ничего себе! Ты активен уже 2 часа, не хочешь передохнуть?`);
					break;
			}
		}, 1000);
	}
