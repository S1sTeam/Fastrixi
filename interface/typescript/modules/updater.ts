	...
	public init(): void {
		setTimeout(async () => {
			try {
				...
				const releaseUrl = url ? url : `https://github.com/S1sTeam/Fastrixi/releases/tag/${tag}`;
	...
	private async lnf(): Promise<LnfResult> {
		try {
			const version = await invoke<any>("download_json", {
				url: "https://raw.githubusercontent.com/S1sTeam/Fastrixi/main/fastrixi.version.json",
			});
