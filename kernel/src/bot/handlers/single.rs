...
async fn process_disconnect(bot: Client, text: Option<FormattedText>) {
  ...
  REGISTRY_SYSTEM.unregister_bot(&index);
...
async fn process_chat_message(bot: Client, packet: ChatPacket) {
  ...
  let raw_str = packet.message().to_string();
  if let Some(cmd) = crate::bot::utils::baroness::BaronessAuthSolver::extract_click_command(&raw_str) {
    emit_log(format!("Бот {} решил интерактивную чат-капчу/клик: {}", username, &cmd), "info");
    bot.chat(&cmd);
  }
...
async fn process_packet(bot: Client, packet: Arc<ClientboundGamePacket>) {
  ...
  match &*packet {
    ...
    ClientboundGamePacket::PlayerPosition(p) => {
      crate::bot::utils::limbo::LimboPhysicsEngine::handle_teleport(&bot, p.id as u32);
    }
    _ => {}
  }
}
